...
namespace
{
    ...
    auto rt11_open(const char *path, struct fuse_file_info *fi) -> int
    {
        return getFS()->open(path, fi);
    }

    auto rt11_create(const char *path, mode_t mode, struct fuse_file_info *fi) -> int
    {
        return getFS()->create(path, mode, fi);
    }

    auto rt11_release(const char *path, struct fuse_file_info *fi) -> int
    {
    ...
    auto rt11_write(const char *path, const char *buf, size_t count, off_t offset, struct fuse_file_info *fi) -> int
    {
        return getFS()->write(path, buf, count, offset, fi);
    }

    auto rt11_fsync(const char *path, int isdatasync, struct fuse_file_info *fi) -> int
    {
        return getFS()->fsync(path, isdatasync, fi);
    }

    void build_oper(struct fuse_operations *oper)
    {
    ...
    {
        std::cerr << "usage: " << program << " disk-image mountpoint\n";
        exit(1);
    }
...
auto main(int argc, char *argv[]) -> int
{
    ...
    if (config.image == NULL) {
        std::cerr << argv[0] << ": must specify an image to mount\n";
        usage(argv[0]);
    }
